...
from utils.generate_report import generar_csv



DESCRIPCION_PUESTO_DEFAULT = """
...
def procesar_entrada():
    ...
    if archivo_cv is not None:
        st.success(f"✅ Archivo cargado: {archivo_cv.name}  |  Tamaño: {archivo_cv.size:,} bytes")
    ...
    descripcion_puesto = st.text_area(
        ...
        height=250,
        placeholder=f"""Ejemplo detallado: {DESCRIPCION_PUESTO_DEFAULT} """,
...
def mostrar_area_resultados():
    ...
    if st.session_state.get('analizar', False):
        ...
        if not descripcion_puesto:
            ...
            #return
            st.info("⚠️ No se proporcionó descripción del puesto. Se usará una descripción por defecto.")
            descripcion_puesto = DESCRIPCION_PUESTO_DEFAULT
...
def procesar_analisis(archivo_cv, descripcion_puesto):
    ...
    with st.spinner("🔄 Procesando currículum..."):
        ...
        status_text.text("📊 Analizando candidato, sólo tomará un momento...")
        progress_bar.progress(25)
        ...
        if resultado.estado_procesamiento:
            mostrar_resultados(resultado)
        else:
            st.error("❌ Hubo un error al procesar el CV. Por favor, intenta con otro archivo.")
...
def mostrar_resultados(resultado: AnalisisCV):
    ...
    with col2:
        st.info(f"**🎓 Educación:** {resultado.nivel_educacion}")
    ...
    with col2:
        st.download_button(
            ...
            data=generar_csv(resultado),
            file_name=f"analisis_cv_{resultado.nombre_candidato.replace(' ', '_')}.csv",
            mime="text/csv"
        )